        ...
        public void Shoot(NPC npc)
        {
            if (counter <= 0)
            {
                ...
                if (Main.myPlayer == Projectile.owner)
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(), Projectile.position, new Vector2(Projectile.DirectionTo(npc.Center).X * 12, Projectile.DirectionTo(npc.Center).Y * 12), ModContent.ProjectileType<FriendlyEyeLaserProj>(), 150, 4, Projectile.owner);
            }
            else
                counter--;
        }
