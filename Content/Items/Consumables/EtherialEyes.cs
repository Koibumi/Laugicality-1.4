...
namespace Laugicality.Content.Items.Consumables
{
	public class EtherialEyes : LaugicalityItem
    {
        ...
        {
            Projectile.NewProjectile(source, position.X, position.Y, speed.X, speed.Y, ModContent.ProjectileType<GeneralBossSpawn>(), NPCID.Retinazer, knockBack, player.whoAmI);
            Projectile.NewProjectile(source, position.X, position.Y, speed.X, speed.Y, ModContent.ProjectileType<GeneralBossSpawn>(), NPCID.Spazmatism, knockBack, player.whoAmI);
            return false;
        }
