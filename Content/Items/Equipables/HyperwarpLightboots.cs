    ...
    public class HyperwarpLightboots : LaugicalityItem
    {
        ...
        private void DashBurst(Player player)
        {
            ...
            for(int i = 0; i < numProj; i++)
            {
                ...
                Projectile.NewProjectile(player.GetSource_FromThis(), player.Center.X, player.Center.Y, mag * (float)Math.Cos(theta), mag * (float)Math.Sin(theta), ModContent.ProjectileType<CrystalliteOrb>(), (int)(28 * player.GetModPlayer<LaugicalityPlayer>().GetGlobalDamage()), 0, player.whoAmI);
            }
        }
