    ...
    public class SuperIchorSpitter : ModNPC
    {
        ...
        private void Shoot(NPC npc)
        {
            ...
            if (_delay > 480)
            {
                _delay = Main.rand.Next(1, 120);
                if (Main.netMode != 1)
                    Projectile.NewProjectile(NPC.GetSource_FromThis(), npc.Center.X, npc.Center.Y, shootDir.X, shootDir.Y, 288/*Hostile Golden Shower*/, (int)(npc.damage / 4), 3, Main.myPlayer);
            }
        }
