    ...
    public class GearSlime : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if(counter > 2 * 60)
            {
                ...
                {
                    for(int i = 0; i < 4; i++)
                        Projectile.NewProjectile(NPC.GetSource_FromThis(), NPC.Center.X, NPC.Center.Y, Main.rand.Next(-4, 4), -3 - Main.rand.Next(3), ModContent.ProjectileType<Gearikan>(), (int)(NPC.damage / 2), 3, Main.myPlayer);
                }
            }
            MovementCheck();
        }
